...
data$drive_after_turnover <- factor(lag(data$turnover, n = 1),
                                    levels = c(0, 1),
                                    labels = c("no", "yes"))
...
baseline_drives <- drive_touchdowns[which(drive_touchdowns$drive_after_turnover == "no"), ] %>% 
  group_by(starting_position) %>% 
  summarize(td_percentage = mean(touchdown))
after_turnover_drives <- drive_touchdowns[which(drive_touchdowns$drive_after_turnover == "yes"), ] %>% 
  group_by(starting_position) %>% 
  summarize(td_percentage = mean(touchdown))
...
ggplot(baseline_drives) +
  ...
  scale_fill_manual(name    = "", 
                    ...
                    labels  = c("Other Drives drives", "Drives following forced turnover")) +
